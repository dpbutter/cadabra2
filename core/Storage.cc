...
//		}
	...
	void Ex::rename(const iterator_base& it, const nset_t::iterator& new_name) {
		if (is_mapped()) nodemap->unmap_node(it);
		it->name=new_name;
		if (is_mapped()) nodemap->map_node(it);
	}

	void Ex::rename(const iterator_base& it, const std::string& str) {
		if (is_mapped()) nodemap->unmap_node(it);
		it->name=name_set.insert(str).first;
		if (is_mapped()) nodemap->map_node(it);
	}

	void Ex::rename(const iterator_base& it, std::string&& str) {
		if (is_mapped()) nodemap->unmap_node(it);
		it->name=name_set.insert(str).first;
		if (is_mapped()) nodemap->map_node(it);
	}
	...
	str_node::str_node(void)
		...
		// Need to initialize the name to some valid string to avoid problems with nodemap.
		name=name_set.insert("").first;
		//	fl.modifier=m_none;
		fl.bracket=b_none;
