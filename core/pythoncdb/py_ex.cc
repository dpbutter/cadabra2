...
//			}
   ...
   Ex_ptr Ex_join(const std::vector<Ex_ptr>& exs)
	...
	void init_ex(py::module& m)
		...
		.def("map_on", [](Ex& ex) { ex.map_on();})
		.def("map_off", [](Ex& ex) { ex.map_off();})
		.def("is_mapped", [](const Ex& ex) { return ex.is_mapped();})
		.def("print_nodemap", [](const Ex& ex) { return ex.print_nodemap();})
